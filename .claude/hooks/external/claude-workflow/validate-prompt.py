# Source: CloudAI-X/claude-workflow
#!/usr/bin/env python3
"""
...
DANGEROUS_PATTERNS = [
    (r"\brm\s+-rf\s+[/~]", "Warning: Recursive delete from root/home detected"),
    (
        r"\bgit\s+push\s+.*--force",
        "Warning: Force push detected - this rewrites history",
    ),
    (r"\bgit\s+reset\s+--hard", "Warning: Hard reset will lose uncommitted changes"),
    (r"\bdrop\s+database\b", "Warning: DROP DATABASE command detected"),
    (r"\btruncate\s+table\b", "Warning: TRUNCATE TABLE will delete all data"),
]
